...
export default function PreferencesScreen() {
  ...
  const savePreferences = async () => {
    ...
    const prefPayload = preferences.flatMap(pref => pref.options).filter(opt => opt.active).map(opt => opt.name)
  ...
  const toggleOption = (categoryIndex: number, optionIndex: number) => {
      ...
      options: cat.options.map((opt, oIdx) => ({
        ...opt,
        active: cIdx === categoryIndex && oIdx === optionIndex ? !opt.active : opt.active,
      })),
    }));
    ...
    <View key={catIndex} style={{ marginBottom: 25 }}>
      ...
      <View style={styles.optionContainer}>
        {item.options.map((opt, optIndex) => (
          ...
          >
            <Text style={opt.active ? { color: '#fff' } : { color: '#202020ff' }}>
              {opt.name}
            </Text>
            <Ionicons
        ...
        <>
          ...
          <View style={styles.buttonContainer}>
            ...
            <TouchableOpacity
              ...
              disabled={isSaving}>
              <Text style={{color:'#ff0a54', fontWeight:'bold'}}>Clear Preferences</Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  optionContainer: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 10, // works in newer RN versions, otherwise use margin
  },
  option: {
    flexDirection: 'row',
    alignItems: 'center',
